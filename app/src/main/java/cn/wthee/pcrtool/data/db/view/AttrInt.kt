...
data class AttrInt(
    ...
    fun all(context: Context = MyApplication.context): ArrayList<AttrValue> {
        ...
        for (i in 0..16) {
            ...
            attrs.add(AttrValue(getString(context, Constants.ATTR[i]), value.toDouble()))
        }
        return attrs
    }
     ...
     * boss 相关属性
     */
    fun enemy(context: Context = MyApplication.context): List<AttrValue> {
        val attrs = all(context)
    ...
    fun multiplePartEnemy(context: Context = MyApplication.context): List<AttrValue> {
        val attrs = all(context)
        val newList = arrayListOf<AttrValue>()
